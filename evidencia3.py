...
print('Bienvenido(a) al negocio de ventas de equipos de computo')
print(separador)
...
def RegistrarVenta():
    ...
    while True:
        folio = int(input(f'Introduzca folio de venta de equipo \n» '))
    ...
    while True:
        descripcion = input('Introduzca descripción del tipo de equipo\n» ')
        cantidadVenta = int(input('Introduzca la cantidad a vender de el tipo de equipo mencionado\n» '))
        precioVenta = float(input('Introduzca precio (sin iva) del tipo de equipo (por unidad)\n» $'))
        print(separador)
        subtotal = (cantidadVenta * precioVenta)
        print(f'Subtotal (sin iva) de los Equipos tipo {descripcion}:','${:.2f}'.format(subtotal))
        print(separador)
        ...
        Agrega_Otro_Equipo_a_la_Misma_Venta  = int(input('¿Desea agregar otra venta de Equipo(s) a la misma venta?\n[1] Si \n[2] No\n» '))

        if Agrega_Otro_Equipo_a_la_Misma_Venta  == 2:
...
def GuardarSQL_Lite3():
    ...
    try:
        with sqlite3.connect("RegistroVentaEquipos.db") as conn: #1 Establezco conexion
